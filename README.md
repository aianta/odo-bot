...
* `git`, to fetch the `canvas-evaluation-scripts` submodule
* Python 3.9 or newer on `PATH`. The Agent-E and WebVoyager runs are scored on the host rather than inside a container, so the evaluation script runs with your local Python. `cascon-environment-setup.bat` checks for it and installs the two packages it needs.


# Installation 

1. Clone this git repository, including its submodule. `git clone --recurse-submodules git@github.com:aianta/odo-bot.git`
...
3. Run `cascon-environment-setup.bat`.
...
`cascon-experiment.bat` runs the experiment with any of the three agents compared in the CASCON 2026 paper — OdoBot, Agent-E or WebVoyager — against the same containerised Canvas environment. All three execute the same 46 Side-effect task instances and produce the same evaluation report format, so their results are directly comparable.
...
>Note: A full 46-task baseline run takes hours, and WebVoyager's network dumps can reach ~19MB per task (roughly 1GB per run).
...
Every run is scored by `evaluation_script.py` from the `canvas-evaluation-scripts` submodule, which matches the HTTP requests an agent made against the `answer_key` patterns in `sample_generated_data/cascon-2026/tasks.json`. Evaluation is entirely offline: it reads artifacts from disk and needs no access to Canvas.
...
>**Important:** the evaluator builds its denominator from the artifacts it *finds*, not from the task list. A task that produced no artifact disappears from the calculation rather than scoring as a failure, so a run that died halfway through would report `20/20` instead of `20/46`. `cascon-experiment.bat` counts the artifacts against the task count and warns when they do not match. This matters most for WebVoyager, which catches every exception and exits `0` even when the batch aborted early — its exit code is not a reliable signal.

>Note: Information-Seeking tasks are out of scope; that path is a stub in `evaluation_script.py`. The 46 instances all three agents run are the Side-effect tasks, which are scored from the captured network requests.
