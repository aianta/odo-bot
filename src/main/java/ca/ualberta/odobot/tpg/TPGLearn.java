...
public class TPGLearn
{
    ...
    public ArrayList<Team> rootTeams = new ArrayList<Team>();
    ...
    public ArrayList<Long> actions = new ArrayList<Long>();
    ...
    public LinkedList<Team> teamQueue = new LinkedList<Team>();

    // Team Population Size
    public int teamPopSize = 0;

    // Root Team Gap Percentage
    public double teamGap = 0.0;

    // Probability of Learner Deletion
    public double probLearnerDelete = 0.0;

    // Probability of Learner Addition
    public double probLearnerAdd = 0.0;

    // Probability of Action Mutation
    public double probMutateAction = 0.0;

    // Probability that when an Action is Mutated it will be a Team
    public double probActionIsTeam = 0.0;

    // Maximum Team Size
    public int maximumTeamSize = 0;

    // Number of Training Epochs
    public long epochs = -1;

    // Learners: Maximum Program Size
    public int maximumProgramSize = 0;

    // Learners: Probability of Instruction Deletion
    public double probProgramDelete = 0.0;

    // Learners: Probability of Instruction Add
    public double probProgramAdd = 0.0;

    // Learners: Probability of Instruction Swap
    public double probProgramSwap = 0.0;

    // Learners: Probability of Instruction Mutate
    public double probProgramMutate = 0.0;

    // Learners: Maximum Action Program Size
    public int maximumActionProgramSize = 0;

    // Learners: Probability of Action Instruction Deletion
    public double probActionProgramDelete = 0.0;

    // Learners: Probability of Action Instruction Add
    public double probActionProgramAdd = 0.0;

    // Learners: Probability of Action Instruction Swap
    public double probActionProgramSwap = 0.0;

    // Learners: Probability of Action Instruction Mutate
    public double probActionProgramMutate = 0.0;

    // The number of registers in an action program.
    public int numberofActionRegisters = 0;

    //every 10 generations we're going to switch between mutating a learner's
    //bid program and a learner's action program. to keep things spicy.
    public boolean mutateBidProgram = false;
    ...
    public TPGLearn( Map<String, String> arguments )
    {
        ...
        maximumActionProgramSize = Integer.valueOf( arguments.get("maximumActionProgramSize") );
        probActionProgramDelete = Double.valueOf( arguments.get("probActionProgramDelete") );
        probActionProgramAdd = Double.valueOf( arguments.get("probActionProgramAdd") );
        probActionProgramSwap = Double.valueOf( arguments.get("probActionProgramSwap") );
        probActionProgramMutate = Double.valueOf( arguments.get("probActionProgramMutate") );
        numberofActionRegisters = Integer.valueOf( arguments.get("numberofActionRegisters") );

        setSeed(Integer.valueOf( arguments.get("seed") ));
        ...
        System.out.println("arg maximumActionProgramSize " + maximumActionProgramSize);
        System.out.println("arg probActionProgramDelete " + probActionProgramDelete);
        System.out.println("arg probActionProgramAdd " + probActionProgramAdd);
        System.out.println("arg probActionProgramSwap " + probActionProgramSwap);
        System.out.println("arg probActionProgramMutate " + probActionProgramMutate);
        System.out.println("arg numberOfActionRegisters " + numberofActionRegisters);

        System.out.println("arg seed " + getSeed());
        System.out.println("\n\n");
    }
    public TPGLearn()
    {
    ...
    public boolean initializePopulations()
    {
        ...
        //this is like this so we don't let them start off with bounty runes and stuff.
        //ArrayList<Long> actions = new ArrayList<Long>(Arrays.asList(0L, 1L, 2L, 3L, 4L, 5L, 6L, 7L, 8L, 15L, 16L, 17L, 18L, 19L, 20L, 21L, 22L, 23L, 24L));
        ...
        for( int i=0; i < teamsToKeep; i++ )
        {
            ...
            // Create a Learner and add it to the Team
            // as well as the Learner population.
            learner = new Learner(-1,action1, numberofActionRegisters, maximumActionProgramSize, maximumProgramSize);
            team.addLearner(learner);
            learners.add(learner);

            // Create another Learner and add it to the Team
            // as well as the Learner population.
            learner = new Learner(-1, action2, numberofActionRegisters, maximumActionProgramSize, maximumProgramSize);
            team.addLearner(learner);
            learners.add(learner);
            ...
            {
                learner = new Learner(-1, actions.get((int)(TPGAlgorithm.RNG.nextDouble() * actions.size())), numberofActionRegisters, maximumActionProgramSize, maximumProgramSize);
                team.addLearner(learner);
                learners.add(learner);
            }
...
//        {
//            ArrayList<Instruction> P = L.getActionObject().actionProgram.program;
//            for(Instruction I : P)
//                if(I.getOperationRegister().equals(Instruction.WRIT))
...
//        }
    ...
    public double[] participate( double[] inputFeatures )
    {
    ...
    public double[] participate( double[] inputFeatures, long[] actions )
    {
        // If we have no Teams left during this learning phase, then we can't learn anything
        if( teamQueue.isEmpty() )
            return null;
        ...
        // Give the team the input features to find an action
        double[] action = team.getAction(new HashSet<Team>(), inputFeatures);

        // If the team's action is in the action list, return it
        for( int i=0; i < actions.length; i++ )
            if( action[action.length - 1] == actions[i] )
                return action;

        // If the action is not in the allowed action list, return a default of 0
        return new double [numberofActionRegisters];
    }

    public boolean incrementFitnessDetail(String detail, double value)
    {
        // If there's no Team to reward, we don't bother
        if( teamQueue.isEmpty() )
            return false;

        teamQueue.peek().incrementFitnessDetail(detail, value);
        return true;
    }
    ...
    public void generateNewTeams(long mutationCount)
    {
        ...
        while( teams.size() < teamPopSize )
        {
            ...
            //long mutationCount = (epochs % 100 == 0 ? 5 : 1);
            //long mutationCount = 5;
    ...
    public boolean mutate( long epoch, Team team )
    {
        ...
        // For every Learner in the ORIGINAL Team, attempt to add Learners and Mutate Bid Programs or Actions.
        ...
        for( Learner l : learnerSet )
        {
            ...
            //are we on a mutate bid cycle or a mutate action cycle?
            if (mutateBidProgram)
            {
            ...
            if( TPGAlgorithm.RNG.nextDouble() < probLearnerAdd )
            {
                ...
                if( TPGAlgorithm.RNG.nextDouble() < probMutateAction )
                {
                    ...
                    if( TPGAlgorithm.RNG.nextDouble() < probActionIsTeam )
                    {
                        ...
                        action.mutate(actionTeam);
                        // Increase the references to this Team
                        actionTeam.increaseReferences();
                    }
                    else {
                        //was the learner's action a team before or an action program before?
                        //if it was a program, we can mutate that program, else, make a new action program.
                        action = new Action( learner.getActionObject());
                        //action.mutate(actions.get((int)(TPGAlgorithm.RNG.nextDouble() * actions.size())));
                        action.mutate(-1L,probActionProgramDelete, probActionProgramAdd, probActionProgramSwap, probActionProgramMutate, maximumActionProgramSize, false);
    ...
    public long nextEpoch()
    {
        ...
        for (Team team : rootTeams)
        {
            //if this team has no outcomes
            if (team.numOutcomes() < 1)
            {
                //add it to the teamQueue
                teamQueue.add(team);
            }
        }
        ...
        if ((epochs % 10) == 0)
        {
            mutateBidProgram = !mutateBidProgram;
        }

        // The number of epochs increases and is returned
        return ++epochs;
    }
    public void saveData(String taskName, String dir)
    {
        //		DataSpitter logger = new DataSpitter(rootTeams, taskName, epochs);
        //
        //		String folder = dir + "results/";
        //
        //		// If the appropriate directory doesn't exist, create it.
        //		if(!Files.isDirectory(Paths.get(folder)))
        //			new File(folder).mkdirs();
        //
        //		//change this depending on what kind of selection you're doing:
        //		//logger.writeData(folder, "absolute");
        //		//logger.writeData(folder, "dominates");
        //		logger.writeData(folder, "dominatedBy");
    ...
    {
        return teamQueue.peek().getID();
    }
    public ArrayList<Team> getRootTeams()
    {
        return this.rootTeams;
    }
