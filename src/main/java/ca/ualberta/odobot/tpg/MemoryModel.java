...
import ca.ualberta.odobot.tpg.util.serializers.MemoryModelSerializer;


@DefaultSerializer(MemoryModelSerializer.class)
...
public class MemoryModel
{
    ...
    public MemoryModel(int rows, int columns, double[][] memory, double[] probabilities)
    {
        this.rows = rows;
        this.columns = columns;
        this.memory = memory;
    ...
    public void write(double[] registers)
    {
        ...
        if(registers.length != columns)
        {
            ...
            System.err.println("MEMORY_MODEL: Dimensions of the model: \ncolumns" + columns + "\nrows: " + rows);
            System.err.println("MEMORY_MODEL: Length of the registers: " + registers.length);
            System.exit(-50);
        }
    ...
    {
        System.out.println("Memory probability array:");
        for (int i = 0; i < probabilities.length; i++)
        {
